function plot_cpu_utilisation_comparision()
%{
		...
		for k=1:size(threads)(1)
			...
			for l=1:size(files)(2)
				...
				for m=1:size(io_sizes)(2)
					...
					avg_fuse_opts_ext4_cpu_util = [];

					avg_ext4_cpu_idle = [];
					avg_fuse_ext4_cpu_idle = [];
					avg_fuse_opts_ext4_cpu_idle = [];
					...
					for n=1:iterations
						...
						filename3 = strcat(inputDir3, 'cpu_utilisation.txt');

						ext4_cpu_util = load(filename1);
						fuse_ext4_cpu_util = load(filename2);
						fuse_opts_ext4_cpu_util = load(filename3);
						%% user : system
						...
						avg_fuse_opts_ext4_cpu_util = [avg_fuse_opts_ext4_cpu_util; mean(fuse_opts_ext4_cpu_util(:, 2)) mean(fuse_opts_ext4_cpu_util(:, 4))];

						filename1 = strcat(inputDir1, 'cpu_idle.txt');
						filename2 = strcat(inputDir2, 'cpu_idle.txt');
						filename3 = strcat(inputDir3, 'cpu_idle.txt');

						ext4_idle = load(filename1);
						fuse_ext4_idle = load(filename2);
						fuse_opts_ext4_idle = load(filename3);
						%% idle : iowait
						...
						avg_fuse_opts_ext4_cpu_idle = [avg_fuse_opts_ext4_cpu_idle ; mean(fuse_opts_ext4_idle(:, 2)) mean(fuse_opts_ext4_idle(:, 3))];
...
%{
				...
				NumGroupsPerAxis = size(plot_cpu_util, 1); %4KB, 32KB, 128KB, 1MB
				...
				hold off;
				outfilename = strcat(outputDir, sprintf('/Cpu-util/%s/Cpu-util-comp-%s-%s-%dth-%s.png', type, work_load_type, work_load_op, thread, file));
				print(outfilename, "-dpng");
				close();

				%Cpu idle comparisions graphs
				NumGroupsPerAxis = size(plot_cpu_idle, 1); %4KB, 32KB, 128KB, 1MB
                                NumStacksPerGroup =  2; %user cpu, system cpu or cpu idle, cpu iowait
                                groupBins = 1:NumGroupsPerAxis;
                                MaxGroupWidth = 0.65; % Fraction of 1. If 1, then we have all bars in groups touching
                                groupOffset = MaxGroupWidth/NumStacksPerGroup;
                                ...
                                title(sprintf('%s %s %d %s', work_load_type, work_load_op, thread, file), 'fontsize', 15);
                                hold off;
                                outfilename = strcat(outputDir, sprintf('/Cpu-idle/%s/Cpu-idle-comp-%s-%s-%dth-%s.png', type, work_load_type, work_load_op, thread, file));
                                print(outfilename, "-dpng");
                                close();
%}
			end
