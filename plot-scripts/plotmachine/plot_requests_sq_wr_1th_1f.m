function plot_requests_sq_wr_1th_1f()
        ...
        for j=1:size(work_load_ops)(2)
			...
			for l=1:size(files)(2)
				...
				a = fix((write_reqs(:, 1))/1000);
                                ...
                                dx=0.5;
                                dy=0.5;
                                text(X-dx, write_reqs(:, 1)+500000, c, 'FontSize', 20);
				...
				ylabel('Fuse Write Requests', 'fontsize', 20);
				...
				title(sprintf('%s %s %d %s Write operations', work_load_type, work_load_op, thread, file), 'fontsize', 15);
                                hold off;
				outfilename = strcat(outputDir, sprintf('/Requests/Write-reqs-comp-%s-%s-%dth-%s.png',  work_load_type, work_load_op, thread, file));
